...
export class Fluid {
  ...
  step(dtFrame, gx, gy, gz) {
    const n = this.n;
    if (n === 0) return;
    // The material's OWN tuning. This read CONFIG.fluid directly for a long
    // time, behind a local alias, so every liquid silently ran on water's
    // timestep, substep budget and solver iteration count no matter what its
    // own config said — invisible until mercury needed more iterations to hold
    // a bead together and changing the number did precisely nothing.
    ...
    for (let s = 0; s < steps; s++) {
      ...
      for (let it = 0; it < this.iterations; it++) {
        // Tension first, incompressibility second, so density has the last
        // word each iteration and the body cannot be crushed.
        this.solveTension();
        this.solveDensity();
  ...
  /**
   * Surface tension, as a CONSTRAINT projected inside the solver loop rather
   * than a force applied after it.
   *
   * The constraint is that a particle should sit at the centroid of its
   * neighbours. In the interior that is already true — a surrounded particle
   * is its own centroid, by symmetry — so this does nothing there. At a
   * surface the neighbourhood is one-sided, the centroid lies inward, and the
   * offset is proportional to how sharply the surface curves. Moving along it
   * is discrete mean-curvature flow, which *is* surface tension: it is the
   * geometry of "shrink the surface", not an approximation of it. Laplace
   * pressure falls out for free, because a tighter curve means a longer offset
   * means a harder pull.
   *
   * Why this works where the force version could not. A force applied after
   * the projection is undone by the next projection, so the two argue across
   * substeps and the argument is where the energy came from — the previous
   * implementation needed a damping term, a shifted kernel and an impulse
   * clamp to stay still, and the clamp then silently became the strength
   * setting. Here tension and incompressibility are solved *together*, a few
   * iterations of each per substep, converging on a position that respects
   * both. Velocity is read out of the net position change exactly as it
   * already is for pressure, so nothing new can inject energy. Density runs
   * last in each iteration, so it always gets the final say and the body
   * cannot be compressed the way the force version crushed it.
   *
   * A wall crops the neighbourhood the same way a free surface does, so the
   * centroid also leans away from glass — non-wetting, for free, from the same
   * term.
   */
  solveTension() {
    const k = this.tuning.tension;
    if (!k) return;
    const n = this.n;
    const { px, py, pz, nbr, nbrCount, dx, dy, dz } = this;
    const cap = this.maxNeighbours;
    const hh = this.smoothing;
    const inv = 1 / hh;
    const maxMove = this.spacing * this.tuning.tensionMaxMove;
    ...
    for (let i = 0; i < n; i++) {
      ...
      for (let m = 0; m < count; m++) {
        const j = nbr[base + m];
        const ox = px[j] - xi, oy = py[j] - yi, oz = pz[j] - zi;
        const r2 = ox * ox + oy * oy + oz * oz;
        if (r2 >= hh * hh) continue;
        // Smooth falloff so a neighbour crossing the radius does not step.
      ...
      if (wsum < 1e-6) { dx[i] = 0; dy[i] = 0; dz[i] = 0; continue; }
      // Offset to the neighbourhood centroid, times stiffness.
      const s = k / wsum;
      let mx = sx * s, my = sy * s, mz = sz * s;
      const m2 = Math.hypot(mx, my, mz);
      if (m2 > maxMove) {
        const t = maxMove / m2;
        mx *= t; my *= t; mz *= t;
      }
      dx[i] = mx; dy[i] = my; dz[i] = mz;
    }

    // Applied in a second pass: every centroid is measured against the same
    // set of positions, so the result does not depend on particle order.
    for (let i = 0; i < n; i++) {
      px[i] += dx[i]; py[i] += dy[i]; pz[i] += dz[i];
    }
    this.project(px, py, pz, n);
  }
